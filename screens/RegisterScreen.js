...
export default function RegisterScreen() {
  ...
  const resendOTP = async () => {
    ...
    try {
      ...
      const url = Network.apiurl + "send_otp";
  ...
  const checkOTP = async () => {
    ...
    if (otp == "" || otp.length < 4) {
      setLoadning(false);
      return alert("Please enter OTP correctly");
    }
  ...
  const signUp = async () => {
    ...
    if (phone.length < 10) {
      setLoadning(false);
      return alert("Phone must be minimum 10 characters");
    }
    ...
    try {
      ...
      const url = Network.apiurl + "register";
  ...
  return (
    <ScrollView
      ...
      style={{ marginTop: Constants.statusBarHeight, backgroundColor: "#fff" }}
    >
